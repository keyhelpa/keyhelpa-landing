...
class Stack extends React.Component {
  ...
  componentDidMount() {
    ...
    if (data && data.proposal.job.merchant && data.proposal.job.merchant.addition_informations) {
      this.setState({
        additionalInformations: JSON.parse(data.proposal.job.merchant.addition_informations)
      })
    }
  }
  ...
  handleCategory(index) {
    const { data } = this.props;
    if (data && data.proposal.job.categories && data.proposal.job.categories.length > 0) {
    ...
    return (
      ...
      >
        <div style={{ paddingLeft: 20 }}
          className="full-width-mobile">
          {
            (data.hourly_rate) && this.renderRow({
              value: Common.getAmountWithCurrency(data.currency, data.hourly_rate) + '/hr',
              label: 'Hourly rate'
            })
          }
        ...
        <div
          ...
          {
            (data.additional_details && data.additional_details.daily_schedule) && this.renderRow({
          ...
          {
            (data.merchant) && this.renderRowPaymentVerified({
  ...
  left(data) {
    return (
      <div style={{
        ...
        {
          (data.hourly_rate) && this.renderRow({
            value: Common.getAmountWithCurrency(data.currency, Number(data.hourly_rate)) + '/hr',
            label: 'Hourly rate'
          })
        }
  ...
  right(data, from) {
    ...
    return (
      <div style={{
          ...
          data && data.proposal && (
            <div style={{
              ...
              <div style={{ marginLeft: -30 }}>
                {
                  this.renderText({
                    title: 'Message to you by Helpa why they are fit for the job.',
                    content: () => {
                      return (
                        <span>
                          <p>
                            {data.proposal.details.message}
                          </p>
                        </span>
                      )
                    }
        ...
        {
          (data.proposal.job.requirements && data.proposal.job.requirements?.tasks && data.proposal.job.requirements?.tasks.length > 0) && this.renderText({
        ...
        {
          (data.proposal.job.requirements && this.renderText({
              ...
              return (
                <div style={{
                  ...
                  <div style={{
                    ...
                    {
                      (data.proposal.job.requirements !== null && data.proposal.job.requirements?.experience.length > 0) && this.category(data.proposal.job.requirements)
                    }
                  </div>
                    ...
                    <div style={{
                      ...
                      {
                        data.proposal.job.requirements !== null && this.leftExperience(data.proposal.job.requirements.cert)
                      }
                    </div>
                    ...
                    <div style={{
                      ...
                      {
                        data.proposal.job.requirements && this.rightExperience(data.proposal.job.requirements.vaccine)
                      }
                    </div>
                  </div>
                  <p style={{ fontWeight: 'bold' }}>
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          {
            (data && data.proposal.job.job_terms) && this.left(data.proposal.job.job_terms)
          }
        </div>
        ...
        <div style={{
          ...
          {
            (data && data.proposal.job.job_terms) && this.renderMobileLeft(data.proposal.job.job_terms)
          }
        </div>
