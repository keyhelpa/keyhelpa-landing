...
const { REACT_APP_URL, REACT_APP_AGENT, REACT_APP_HELPA, REACT_APP_HOST } =
  process.env;
...
const links = [
  {
    ...
    route:
      mainRoute +
      `${Helper.ACCOUNT_TYPE === "Freelancer" ? "helpa/about" : "agent/about"}`,
  },
  {
    title: "Contact Us",
    option: "external",
    route:
      mainRoute +
      `${
        Helper.ACCOUNT_TYPE === "Freelancer"
          ? "helpa/contact_us"
          : "agent/contact_us"
      }`,
  },
      ...
      >
        ...
        >
          {Strings.socialMedias.map((item, index) => (
            <span
              key={index}
              style={{ ...style.icon, backgroundColor: Colors.footerIcons }}
              className="cursor-hover"
              onClick={() => {
                window.location.href = item.route;
              }}
