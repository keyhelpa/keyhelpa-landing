...
import { faFacebookF, faLinkedinIn, faYoutube } from '@fortawesome/fontawesome-free-brands'
...
class Stack extends React.Component {
  ...
  acceptFreelancer() {
    ...
    API.request(Routes.contractCreate, parameter, response => {
      if (response.data !== null) {
        this.setState({
          hiredItem: null,
          acceptModal: false
  ...
  renderActions() {
    ...
    return (
      ...
      >
        {
          data && data.proposal && (data.proposal.category == 'invite' && (data.proposal.category_status === 'pending')) && (
            <Button
              title={'Cancel Invite'}
              onClick={() => {
        ...
        {
          data && data.proposal && (data.proposal.category == 'invite' && (data.proposal.category_status === 'pending' || data.proposal.category_status === 'declined')) && (
            <Button
              title={'Resend Invite'}
              onClick={() => {
              ...
              style={{
                backgroundColor: data.proposal.category_status === 'pending' ? Colors.primary : 'transparent',
                color: data.proposal.category_status === 'pending' ? Colors.white : Colors.primary,
                border: 'solid 1px ' + Colors.primary,
                marginRight: data.proposal.category_status === 'pending' ? 0 : 20
              }}
        ...
        {
          data && data.proposal && ((data.proposal.category == 'proposals' && data.proposal.category_status !== 'rejected') || (data.proposal.category == 'invite' && data.proposal.category_status === 'accepted')) && (
            <Button
        ...
        {
          data && data.proposal && ((data.proposal.category == 'proposals' && data.proposal.category_status === 'rejected') || (data.proposal.category == 'invite' && data.proposal.category_status === 'declined')) && (
            <Button
              title={'Remove'}
              onClick={() => {
                this.setState({
        ...
        {
          data && data.proposal && (data.proposal.category == 'interview' && data.proposal.category_status != 'hired') && (
            <Button
              title={'Reject'}
              onClick={() => {
                this.setState({
  ...
  render() {
    const { data } = this.props;
    const { hireModal, modal, selectInterview, jobs, rejectInterview, removeModal, removeInterview, cancelInvite, inviteFlag, removedItem, acceptModal } = this.state;
    return (
      <div style={{
        width: '100%',
        borderRadius: 12,
        ...
        }
        {
          selectInterview && (
            <Interview
              show={true}
              title={'Interview'}
              subTitle={'Send an interview details to Helpa to discuss about this job you are hiring.'}
...

        {
          removeModal && (
            <ConfirmationModal
              show={removeModal}
              title="Confirmation"
              message={`Are you sure you want to remove this ${removedItem && removedItem.category === 'invite' ? 'invite' : 'proposal'}?`}
              onCancel={() => {
                this.setState({
                  removeModal: false
                })
