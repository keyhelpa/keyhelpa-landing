...
class Stack extends React.Component {
      ...
      (response) => {
        ...
        if (response.data.length > 0) {
          let temp = response.data.reverse().map((item) => {
            return {
              ...item,
              title: item.account.information
                ? item.account.information.first_name +
                  " " +
          ...
          this.setState(
            ...
            () => {
              this.props.setMessages(
                flag === false ? temp : _.uniqBy([...data, ...temp], "id")
              );
            }
          );
        } else {
  ...
  renderProfile(item) {
    return (
      ...
      >
        {item.account && item.account.profile ? (
          <img
            src={
              item.account.profile.url.includes("storage")
                ? REACT_APP_API_URL + item.account.profile.url
                : item.account.profile.url
            }
            style={{
              width: 50,
              height: 50,
          ...
          {item.payload === "text" && (
            <span
              ...
              dangerouslySetInnerHTML={{
                __html: this.convertLineToBreak(item.message),
              }}
            ></span>
          )}
          {item.payload === "image" &&
            item.files?.length > 0 &&
            item.files.map((iItem, index) => (
              <img
                src={
                  iItem.url.includes("storage")
                    ? REACT_APP_API_URL + iItem.url
                    : iItem.url
                }
                style={{
                  height: 200,
