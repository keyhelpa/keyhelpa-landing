...
function Features(props) {
  ...
  const getImage = (template, device) => {
    if (device === "desktop") {
      switch (template) {
        case "up":
          return AgentCircleUp;
        case "down":
          return AgentCircleDown;
        case "circle":
          return AgentCircle;
        case "up-pink":
          return HelpaCircleUp;
        case "down-pink":
          return HelpaCircleDown;
        case "circle-pink":
          return HelpaCircle;
      }
    } else {
      switch (template) {
        case "up":
          return AgentMobileCircleRight;
        case "down":
          return AgentMobileCircleLeft;
        case "circle":
          return AgentMobileCircle;
        case "up-pink":
          return HelpaMobileCircleRight;
        case "down-pink":
          return HelpaMobileCircleLeft;
        case "circle-pink":
          return HelpaMobileCircle;
      }
    }
  };
      ...
      >
        {data &&
          data.map((item, index) => (
            ...
            >
              {(item.template.includes("down") ||
                item.template.includes("circle")) &&
                renderCircle(item.title, item.template)}
              <div
                style={{
                  float: "left",
                  width: "100%",
      ...
      >
        {data &&
          data.map((item, index) => (
            ...
            >
              {item.template.includes("up") &&
                renderCircleMobile(item.title, item.template, "mobile")}
              <div
                style={{
                  float: "left",
                  width: "50%",
  ...
  const renderBottom = () => {
    return (
      ...
      >
        ...
        >
          Lifestyle. Choose your days, hours.{" "}
          <p style={{ color: "rgb(230, 45, 126)" }}>Be your own boss.</p>
        </h1>
      </div>
    );
  };
  return (
    ...
    >
      ...
      >
        ...
        <div className="hide-on-desktop">{renderMobile(data)}</div>
      </div>
