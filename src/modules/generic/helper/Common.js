...
export default {
    ...
    getReceiveFeeCalculation(fee, rate, currency) {
        ...
        if (fee.type == 'percentage') {
            return this.getCurrencySymbol(currency) + ' ' + parseFloat(rate - ((fee.fee / 100) * rate)).toFixed(2)
        }
        return this.getCurrencySymbol(currency) + ' ' + parseFloat(rate - fee.fee).toFixed(2)
    },
    getFeeCalculation(fee, rate, currency) {
        // let fee = this.KEYHELPA_RATE
        if (fee.type == 'percentage') {
            return this.getCurrencySymbol(currency) + ' ' + parseFloat((fee.fee / 100) * rate).toFixed(2)
        }
        return this.getCurrencySymbol(currency) + ' ' + parseFloat(fee.fee).toFixed(2)
    },
    ...
    getAmountWithCurrency(currency, amount) {
        ...
        return this.getCurrencySymbol(currency) + ' ' + parseFloat(amount).toFixed(2)
    },
    toSentenceCase(word) {
        if (word) {
    ...
    getCompleteName(information) {
        return information.first_name && information.last_name ? this.toSentenceCase(information.first_name) + ' ' + this.toSentenceCase(information.last_name) : ''
    },
    setColor() {
    ...
    getContractValue(terms) {
        if (terms && terms.additional_details && terms.additional_details.total_value) {
            return this.getAmountWithCurrency(terms.currency, terms.additional_details.total_value)
        }
    ...
    getSocialMedia(data) {
        ...
        for (var key in links) {
            if(links.hasOwnProperty(key) && (links[key] != null && links[key] != 'null')){
                array.push({
                    title: key,
                    url: links[key],
