...
class Stack extends React.Component {
  ...
  componentDidMount() {
    ...
    API.request(Routes.accountRetrive, {
      condition: [{
        value: user.id,
        column: 'id',
        clause: '=' 
  ...
  submit(){
    const { old_password, new_password, code, username, email, errorPassword, password, confirmPassword, newPassword, confirmNewPassword, errorConfirmPassword } = this.state;
    const { user } = this.props.state;
  ...
  render() {
    const { password, errorPassword, confirmPassword, errorConfirmPassword, errorNewPassword, newPassword, passwordShowFlag, confirmPasswordShowFlag, newPasswordShowFlag } = this.state;
    return (
      <div
        style={{ width: '100%' }}>
        <BreadCrumbs
