...
class Stack extends React.Component {
  ...
  retrieveContracts() {
    ...
    API.request(
      Routes.contractRetrieveJobSummary,
      {
        condition: [
          {
            value: user.id,
            column:
              user.account_type.toLowerCase() == "freelancer"
                ? "freelancer"
                : "account_id",
            clause: "=",
          },
  ...
  getSelectionData(data) {
    if (data) {
      let newData = data.map((item) => {
        return item.job && item.job.merchant
          ? item.job.title + " - " + item.job.merchant.name
          : "";
      });
      return newData;
    }
  ...
  renderSelectedUser(account) {
    ...
    return (
      ...
      >
        ...
        >
          <ProfilePicture size={50} iconSize={"lg"} data={account.profile} />
          <b
            style={{
              paddingLeft: 10,
            }}
  ...
  renderCreateTask() {
    ...
    return (
      ...
      >
        ...
        {createTaskLoading && <Skeleton height={50} />}
      </div>
    );
  }
  ...
  renderCard(data, list) {
    ...
    return (
      ...
      >
        {createTask &&
          createTask.status == list.variable &&
          ...
          >
            ...
            >
              {item.members &&
                ...
                item.members.map((iMember) => (
                  <div
                    className="cursor-hover"
                    onClick={() => {
                      this.setState({
                        selectedUser:
                          iMember && iMember.id == item.id ? null : iMember,
                      });
                    }}
                  >
                    <ProfilePicture
              ...
              {selectedUser &&
                ...
                this.renderSelectedUser(selectedUser.account)}
            </div>
          </div>
        ))}
      </div>
  ...
  renderList() {
    ...
    return (
      ...
      >
        {list.map((item) => (
          <div style={style.list} className="full-width-mobile mb-mobile-25">
            <div
              style={{
                width: "100%",
                float: "left",
            ...
            >
              ...
              {taskLoading && <Skeleton height={100} />}
            </div>
          </div>
        ))}
      </div>
