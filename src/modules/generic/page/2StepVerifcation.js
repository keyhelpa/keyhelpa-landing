		...
		return (
			<div>
				...
				<div style={{
					...
					}}>Didn't get verification code? </span>
					<span style={{
			...
			<div style={Style.pageWithBackground}>
				...
				<div style={Style.cardStyle} className='full-width-mobile-with-margin'>
					<HeaderLabel
						...
						description={'Please enter valid value for the field below to continue'}
						_color={true}
					/>
					<p>Hello <b>{user?.username}</b></p>
					{
						platform == 'sms' && (
							<p>We send an otp to your phone number:  ******{user?.information?.cellular_number.slice(-4)}</p>
						)
					}
					{
						platform == 'google_auth' && (
							<p>Please input the code that google generate in your google authenticator app</p>
						)
					}
					<div
						style={{
					...
					>
						...
						{
							platform == 'google_auth' && data && data[0].google_auth == 1 && this.renderGoogleAuth()
						}
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(StepVerifcation));