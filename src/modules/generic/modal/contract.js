...
class Stack extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      terms: [
        {
          title:
            "I authorise that you release the weekly payment held in escrow",
        },
        {
          title:
            "I authorise KeyHelpa to refund the agent all funds held in the escrow account.",
        },
      ],
      selectedTerms: [],
  ...
  onClick() {
    const { reasons, selectedTerms, selectedOption, feedback, ratings } =
      this.state;
    this.setState({
      errorMessage: null,
    });
  ...
  renderBody() {
    ...
    return (
      ...
      >
        ...
        {[
          `You'll be prompted to provide the reason for ${
            this.props.message ? this.props.message : " ending this job"
          } and give feedback.`,
        ].map((item, index) => (
          <p
            key={index}
