...
class Stack extends React.Component {
  ...
  handleCategory(index) {
    const { data } = this.props;
    let category =
      data.category === undefined ? data.categories : data.category;
    ...
    return (
      ...
      >
          ...
          {data.hourly_rate &&
            this.renderRow({
              value:
                ...
                (data.additional_details.selected_rate_type === "hourly_rate"
                  ? "/hr"
                  : ""),
              label:
                data.additional_details.selected_rate_type === "hourly_rate"
        ...
        <div style={{ width: "100%" }}>
          ...
          {data.additional_details &&
            data.additional_details.time_slot &&
            this.renderRow({
          ...
          {this.renderRowPaymentVerified({
            value:
              merchant && merchant.payment_verified
      ...
      >
        {data &&
          ...
          ((data.proposal.category != "invite" &&
            data.proposal.category_status !== "archived") ||
            (data.proposal.category === "invite" &&
              data.proposal.category_status === "accepted")) &&
          (data.proposal.category_status !== "rejected" ||
            data.proposal.category_status !== "cancelled" ||
            data.proposal.category_status !== "archived") && (
            <div
              style={{
                padding: 30,
            ...
            >
              ...
              >
                <b>KeyHelpa's service fee is {this.state.admin_fees}%</b>
              ...
              <div style={{ marginLeft: -30 }}>
                {this.renderText(
                  {
                    title:
                      "Message to hiring agent why you are fit for the job.",
                    content: () => {
                      return (
                        <span>
                          <p>
                            {data.proposal.details !== undefined
                              ? data.proposal.details.message
                              : data.proposal.additional_details.message}
                          </p>
                        </span>
                      );
                    },
        ...
        {data.requirements &&
          ...
          data.requirements?.tasks.length > 0 &&
          this.renderText({
                ...
                >
                  ...
                  >
                    {data.requirements?.experience.length > 0 &&
                      this.category(data.requirements.experience)}
                  </div>
                  ...
                  >
                    ...
                    >
                      {data.requirements &&
                        data.requirements.cert &&
                        this.leftExperience(data.requirements.cert)}
                    </div>
                    ...
                    >
                      {data.requirements &&
                        data.requirements.vaccine &&
                        this.rightExperience(data.requirements.vaccine)}
                    </div>
                  </div>
                  ...
                  <div>
                    <ol>
                      {data &&
                        data?.job_terms?.daily_schedule &&
                        data?.job_terms?.daily_schedule.length > 0 &&
                        data?.job_terms?.daily_schedule.map((el, ndx) => (
                          <li
                            key={ndx}
                            style={{
                              borderSpacing: "10px",
    ...
    return (
      ...
      >
        ...
        >
          {data &&
            data.job_terms &&
            this.renderMobileLeft(data.job_terms, data.merchant)}
        </div>
