...
class Stack extends React.Component {
  ...
  update(param) {
    ...
    let cons = terms.filter((el) => {
      return el.value === "conditions";
    });
    let serve = terms.filter((el) => {
      return el.value === "service_fee";
    });
    let percentage = terms.filter((el) => {
      return el.value === "percent";
    });
    if (data && data.proposal == null) return;
  ...
  submit(param) {
    const { message, terms, image, con, service, hourlyRate, currency } =
      this.state;
    this.setState({
      errorMessage: null,
    });
    ...
    if (cons.length === 0 || serve.length === 0 || percentage.length === 0) {
      this.setState({
        errorMessage:
          "Please accept and acknowledge the following terms and conditions.",
      });
      return;
    }
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            In less than 150 words tell the hiring agent why you’re the perfect
            fit for the job.
          </p>
        </div>
        ...
        >
          <CheckBox
            data={[
              {
                title:
                  "Yes, I understand and agree to KeyHelpa's Terms of Conditions, including the Privacy Policy",
                value: "conditions",
                component: () => {
                  return (
    ...
    return (
      ...
      >
        <ModalHeader
          ...
          subTitle={
            (data && data.proposal ? "Update" : "Submit") +
            " proposal for job" +
            ' "' +
            this.getTitle(data) +
            '" '
